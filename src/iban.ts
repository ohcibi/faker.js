...
export const pattern10 = ['01', '02', '03', '04', '05', '06', '07', '08', '09'];

export const pattern100 = ['001', '002', '003', '004', '005', '006', '007', '008', '009'];
...
export function toDigitString(str: string): string {
  return str.replace(/[A-Z]/gi, match => {
    return `${match.toUpperCase().charCodeAt(0) - 55}`;
  });
}

export function mod97(digitStr: string): number {
  let m = 0;
  for (let i = 0; i < digitStr.length; i++) {
    m = (m * 10 + (parseInt(digitStr[i]) | 0)) % 97;
  }
  return m;
}
